...
fn bench_batch_apply(batch_size: usize, num_batches: usize) -> (Duration, f64) {
    ...
    for entry in &entries {
        let _ = futures::executor::block_on(store.apply_to_state_machine(&[entry.clone()])).unwrap();
    }
    let elapsed = start.elapsed();
    ...
    let handles: Vec<_> = (0..num_threads)
        .map(|t| {
            ...
            std::thread::spawn(move || {
                ...
                for chunk in entries.chunks(100) {
                    let _ = futures::executor::block_on(store.append_to_log(chunk.to_vec())).unwrap();
                }
            })
        })
        .collect();
