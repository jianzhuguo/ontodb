    ...
    pub fn filter_local(&self, target: ShardTarget) -> ShardTarget {
        match target {
            ...
            ShardTarget::Multi(shards) => {
                let local: Vec<ShardId> = shards.into_iter().filter(|s| self.is_local(*s)).collect();
                ShardTarget::Multi(local)
            }
            ShardTarget::All => {
                ShardTarget::Multi(self.local_shards.clone())
            }
        }
    }
...
mod tests {
    ...
    fn test_filter_local() {
        let router = make_router();
        // Shard 1 is local
        assert_eq!(router.filter_local(ShardTarget::Single(1)), ShardTarget::Single(1));
        // Shard 2 is NOT local
        assert_eq!(router.filter_local(ShardTarget::Single(2)), ShardTarget::Multi(vec![]));
        // All → only local shards
        assert_eq!(router.filter_local(ShardTarget::All), ShardTarget::Multi(vec![0, 1]));
    }
