...
fn bench_vertex_crud(store: &GraphStore, n: usize) {
    ...
    println!(
        "  Vertex CRUD: write={:.2}s ({:.0}/s), read={:.2}s ({:.0}/s)",
        write_time.as_secs_f64(),
        n as f64 / write_time.as_secs_f64(),
        read_time.as_secs_f64(),
        n as f64 / read_time.as_secs_f64()
    );
}
...
fn bench_edge_crud(store: &GraphStore, n: usize) {
    ...
    println!(
        "  Edge CRUD: write={:.2}s ({:.0}/s), read={:.2}s ({:.0}/s)",
        write_time.as_secs_f64(),
        n as f64 / write_time.as_secs_f64(),
        read_time.as_secs_f64(),
        n as f64 / read_time.as_secs_f64()
...
fn bench_traversal(store: &GraphStore, num_queries: usize, max_depth: usize) {
    ...
    for _ in 0..num_queries {
        ...
        let neighbors = engine
            .hop(&vid, Direction::Out, Some("KNOWS"), None, None)
            .unwrap();
    ...
    for _ in 0..num_queries {
        let vid = format!("v{}", rng.gen_range(0..num_vertices));
        let result = engine
            .traverse_bfs(&vid, max_depth, Direction::Out, Some("KNOWS"), None)
            .unwrap();
    ...
    println!(
        "  Single-hop: {:.2}s ({:.0} qps), avg_neighbors={:.1}",
        hop1_time.as_secs_f64(),
        num_queries as f64 / hop1_time.as_secs_f64(),
        total_neighbors as f64 / num_queries as f64
    );
    println!(
        "  {}-hop BFS: {:.2}s ({:.0} qps), avg_visited={:.1}",
        max_depth,
        bfs_time.as_secs_f64(),
        num_queries as f64 / bfs_time.as_secs_f64(),
        total_visited as f64 / num_queries as f64
    );
    println!(
        "  {}-hop BFS (fast): {:.2}s ({:.0} qps), avg_visited={:.1}",
        max_depth,
        bfs_fast_time.as_secs_f64(),
        num_queries as f64 / bfs_fast_time.as_secs_f64(),
        total_fast_visited as f64 / num_queries as f64
    );
    println!(
        "  {}-hop BFS (with paths): {:.2}s ({:.0} qps), avg_visited={:.1}",
        max_depth,
        bfs_path_time.as_secs_f64(),
        num_queries as f64 / bfs_path_time.as_secs_f64(),
...
fn bench_graph_vector_hybrid(store: &GraphStore, num_queries: usize) {
    ...
    for _ in 0..num_queries {
        ...
        let _results = index.search_filtered(
            &query_vec,
            5,
            &neighbor_ids
                .iter()
    ...
    println!(
        "  Graph+Vector hybrid: {:.2}s ({:.0} qps)",
        hybrid_time.as_secs_f64(),
        num_queries as f64 / hybrid_time.as_secs_f64()
    );
}
