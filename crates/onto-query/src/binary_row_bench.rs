...
mod tests {
    ...
    fn bench_binary_row_integration() {
        ...
        let cases: &[(&str, &str)] = &[
            ...
            (
                "simple filter (price > 5000)",
                "SELECT * FROM Product WHERE price > 5000",
            ),
            (
                "compound filter (category + price)",
        ...
        let d = bench_query_read(
            &executor,
            "MATCH (p:Product) WHERE p.price > 5000 RETURN p.name",
            BENCH_ITERS,
        );
