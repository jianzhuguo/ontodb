...
impl OntoRaftNetwork {
    ...
    async fn send_rpc<Req: serde::Serialize, Resp: serde::de::DeserializeOwned>(
        ...
        stream.write_all(&len).await.map_err(|e| NetworkError::new(&e))?;
        stream.write_all(&req_bytes).await.map_err(|e| NetworkError::new(&e))?;
        ...
        stream.read_exact(&mut resp_buf).await.map_err(|e| NetworkError::new(&e))?;
