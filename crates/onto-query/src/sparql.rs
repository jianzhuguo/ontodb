...
impl SparqlParser {
    ...
    fn parse_select(&self, input: &str) -> Result<SparqlQuery, String> {
        ...
        // Parse WHERE clause
        let (patterns, filters, optional, unions, remaining) =
            self.parse_where_clause(where_part)?;
    ...
    fn parse_construct(&self, input: &str) -> Result<SparqlQuery, String> {
        ...
        // Parse WHERE clause
        let (patterns, filters, _optional, _unions, _remaining) =
            self.parse_where_clause(where_part)?;
    ...
    > {
        ...
        let end = self
            .find_matching_brace(&input[start..])
            .ok_or("Missing } in WHERE clause")?;
        if start + 1 >= start + end {
            return Err("Invalid WHERE clause: empty or malformed braces".to_string());
        }
        let body = &input[start + 1..start + end];

        let (main_patterns, filters, optional_blocks, union_blocks) =
            self.parse_where_body(body)?;
        ...
        for segment in &segments {
            match segment {
                ...
                WhereSegment::Optional(body) => {
                    let pats = self.parse_triple_patterns(body)?;
                    let filts = self.parse_filters(body)?;
                    optional_blocks.push(OptionalBlock {
                        patterns: pats,
                        filters: filts,
                    });
                }
    ...
    fn find_operator(&self, expr: &str, op: &str) -> Option<usize> {
        ...
        for i in 0..bytes.len() {
            match bytes[i] as char {
                ...
                ')' => depth -= 1,
                '"' => in_string = !in_string,
                _ if depth == 0 && !in_string && bytes[i..].starts_with(op_bytes) => {
    ...
    fn parse_order_by(&self, input: &str) -> Result<(SparqlOrderBy, String), String> {
        ...
        while pos < after.len() {
            ...
            if (remaining.len() >= 5
                && remaining[..5].eq_ignore_ascii_case("LIMIT")
                && (remaining.len() == 5 || !remaining.as_bytes()[5].is_ascii_alphanumeric()))
                || (remaining.len() >= 6
                    && remaining[..6].eq_ignore_ascii_case("OFFSET")
            ...
            if remaining.starts_with('?') {
                ...
                || (remaining.len() >= 3
                    && remaining[..3].eq_ignore_ascii_case("ASC")
    ...
    fn parse_limit(&self, input: &str) -> Result<(usize, String), String> {
        ...
        let limit: usize = num_str
            .parse()
            .map_err(|_| format!("Invalid LIMIT: {}", num_str))?;
    ...
    fn parse_offset(&self, input: &str) -> Result<(usize, String), String> {
        ...
        let num_str = after.split_whitespace().next().unwrap_or("");
        let offset: usize = num_str
            .parse()
            .map_err(|_| format!("Invalid OFFSET: {}", num_str))?;
    ...
    pub fn translate_to_sql(&self, query: &SparqlQuery) -> Result<String, String> {
        ...
        let col_exprs: Vec<String> = select_cols
            .iter()
            .map(|var| {
        ...
        let safe_class: String = class_name
            .chars()
            .filter(|c| c.is_alphanumeric() || *c == '_')
            .collect();
        ...
        if let Some(ref order_by) = query.order_by {
            if !order_by.terms.is_empty() {
                ...
                let order_terms: Vec<String> = order_by
                    .terms
                    .iter()
                    .map(|t| {
                        format!(
    ...
    fn translate_filter(&self, filter: &SparqlFilter) -> Option<String> {
        match filter {
            ...
            SparqlFilter::Regex(var, pattern) => {
                ...
                let conds: Vec<String> = like_patterns
                    .iter()
            ...
            SparqlFilter::Bound(var) => Some(format!("\"{}\" IS NOT NULL", var)),
...
fn regex_to_like(pattern: &str) -> Vec<String> {
    ...
    branches
        ...
        .map(|branch| {
            ...
            let has_complex = branch.contains("\\")
                || branch.contains("[")
                || branch.contains("{")
                || branch.contains("(")
                || branch.contains("+")
...
pub fn format_sparql_json(
    ...
    SparqlResult {
        head: SparqlHead {
            vars: vars.iter().map(|v| format!("?{}", v)).collect(),
        },
...
"#;
        ...
        assert!(
            matches!(q.select, SparqlSelect::Variables(vars) if vars.contains(&"name".to_string()))
        );
    }
...
"#;
        ...
        assert!(
            sql.contains("Person"),
            "SQL should reference Person class: {}",
            sql
        );
    }
...
"#;
        ...
        assert_eq!(
            q.optional[0].patterns.len(),
            1,
            "OPTIONAL should have 1 pattern"
        );
    }
...
"#;
        ...
        assert!(
            sql.contains("\"email\""),
            "SQL should include optional email column: {}",
            sql
        );
        // Main pattern should still have IS NOT NULL
        assert!(
            sql.contains("\"name\" IS NOT NULL"),
            "SQL should require name IS NOT NULL: {}",
            sql
        );
        // Should NOT have email IS NOT NULL (it's optional)
        assert!(
            !sql.contains("\"email\" IS NOT NULL"),
            "SQL should NOT require email IS NOT NULL: {}",
            sql
        );
    }
...
"#;
        ...
        assert!(
            sql.contains("Person"),
            "SQL should reference Person: {}",
            sql
        );
        assert!(
            sql.contains("Organization"),
            "SQL should reference Organization: {}",
            sql
        );
    }
...
"#;
        ...
        assert!(
            matches!(q.filters[0], SparqlFilter::Exists(_)),
            "Should be Exists filter"
        );
    }
...
"#;
        ...
        assert_eq!(q.filters.len(), 1, "Should have 1 filter");
        assert!(
            matches!(q.filters[0], SparqlFilter::NotExists(_)),
            "Should be NotExists filter"
        );
    }
...
"#;
        ...
        assert!(
            sql.contains("\"email\" IS NOT NULL"),
            "EXISTS should translate to IS NOT NULL: {}",
            sql
        );
    }
...
"#;
        ...
        let sql = parser.translate_to_sql(&q).unwrap();
        assert!(
            sql.contains("\"email\" IS NULL"),
            "NOT EXISTS should translate to IS NULL: {}",
            sql
        );
    }
...
"#;
        ...
        assert_eq!(
            q.optional[0].filters.len(),
            1,
            "OPTIONAL block should have 1 filter"
        );
    }
}
