...
打开浏览器访问 `http://127.0.0.1:7912/console`，支持：
- SQL 编辑器（语法高亮）
- 查询历史
- Schema 浏览
- 实时指标
...
| `--master-key-source` | 主密钥来源 | `env:ONTO_MASTER_KEY` |
...
-- 查询所有 Animal（Dog 自动包含在内）
SELECT * FROM Animal;
-- 结果包含：旺财（Dog 是 Animal 的子类）
...
OntoDB 支持 7 条 OWL 2 RL 推理规则：

| 规则 | 说明 | 示例 |
|------|------|------|
| CaxSco | 类继承推理 | Dog ⊑ Animal → 旺财 ∈ Animal |
| CaxEqc | 等价类推理 | Dog ≡ Canine → 旺财 ∈ Canine |
| PrpSpo | 属性继承推理 | hasOwner ⊑ hasBelonging → 传递 |
| PrpEqp | 等价属性推理 | hasName ≡ getName → 语义别名 |
| PrpInv | 反向属性推理 | owns ↔ ownedBy |
| PrpTrp | 传递属性推理 | ancestorOf 传递 |
| PrpSymp | 对称属性推理 | friendOf 对称 |
...
# - ontodb_cache_hits: 缓存命中数
# - ontodb_memtable_size_bytes: MemTable 大小
# - ontodb_disk_usage_bytes: 磁盘使用量
```
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| 连接被拒绝 | 服务器未启动或端口错误 | 检查 `ps aux | grep ontodb` 和端口 |
| 查询超时 | 查询太复杂或数据量太大 | 添加 LIMIT，优化 WHERE 条件 |
| 内存不足 | MemTable 或 Cache 太大 | 减小 `memtable_size_mb` 和 `block_cache_mb` |
| 磁盘满 | WAL 或 SSTable 累积 | 清理旧数据或扩容磁盘 |
| 认证失败 | API Key 错误 | 检查 `--api-key` 配置 |
