...
impl ShardManager {
    ...
    pub fn create_migration(
        ...
        match self.shard_status.get(&source_shard) {
            Some(ShardStatus::Active) => {}
            Some(status) => return Err(format!("Source shard {} is not active (status: {:?})", source_shard, status)),
        ...
        let migration_id = format!("mig_{}_{}_{}", source_shard, target_shard, self.migrations.len());
        ...
        self.shard_status.insert(source_shard, ShardStatus::Migrating { target_node: target_shard as u64 });
    ...
    pub fn update_migration_progress(
        ...
        let task = self.migrations.get_mut(migration_id)
    ...
    pub fn complete_migration_task(&mut self, migration_id: &str) -> Result<MigrationResult, String> {
        ...
        self.shard_status.insert(task.source_shard, ShardStatus::Active);
        ...
        Ok(MigrationResult {
            ...
            records_migrated: match &task.status {
                MigrationStatus::InProgress { migrated_records, .. } => *migrated_records,
                _ => 0,
            },
    ...
    pub fn fail_migration(&mut self, migration_id: &str, error: &str) -> Result<(), String> {
        ...
        task.status = MigrationStatus::Failed { error: error.to_string() };
        task.completed_at = Some(timestamp);
    ...
    pub fn cancel_migration(&mut self, migration_id: &str) -> Result<(), String> {
        ...
        match &task.status {
            ...
            _ => Err(format!("Migration {} cannot be cancelled (status: {:?})", migration_id, task.status)),
        }
    }
    ...
    pub fn active_migrations(&self) -> Vec<&MigrationTask> {
        self.migrations.values()
            .filter(|t| matches!(t.status, MigrationStatus::Pending | MigrationStatus::InProgress { .. }))
            .collect()
    }
    ...
    pub fn remove_shard_with_migration(
        ...
        let classes_to_migrate: Vec<String> = self.shard_map.class_strategies.iter()
            .filter_map(|(class, strategy)| {
                match strategy {
        ...
        Ok(format!("Shard {} removed, {} classes migrated to shard {}", shard_id, classes_to_migrate.len(), target_shard))
    }
    ...
    pub fn split_shard(
        ...
        self.shard_status.insert(source_shard, ShardStatus::Splitting { new_shards: new_shard_ids.clone() });
    ...
    pub fn shard_statistics(&self) -> HashMap<ShardId, ShardStatistics> {
        ...
        for (id, status) in &self.shard_status {
            stats.insert(*id, ShardStatistics {
                shard_id: *id,
                status: status.clone(),
                classes_count: self.shard_map.class_strategies.values()
                    .filter(|s| match s {
