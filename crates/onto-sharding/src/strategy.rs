...
impl ShardStrategy {
    ...
    pub fn route_key(&self, key: &[u8]) -> ShardTarget {
        match self {
            ...
            ShardStrategy::HashBased { num_shards, slot_map } => {
...
mod tests {
    ...
    fn test_range_based_routing() {
        let strategy = ShardStrategy::RangeBased {
            ranges: vec![
                RangeShard { end_key: b"m".to_vec(), shard: 0 },
                RangeShard { end_key: b"t".to_vec(), shard: 1 },
                RangeShard { end_key: b"z".to_vec(), shard: 2 },
            ],
        };
        // "a" < "m" → shard 0
    ...
    fn test_shard_map_class_assignment() {
        ...
        assert_eq!(map.get_strategy("User"), ShardStrategy::ClassBased { shard: 1 });
        assert_eq!(map.get_strategy("Order"), ShardStrategy::ClassBased { shard: 2 });
        assert_eq!(map.get_strategy("Product"), ShardStrategy::ClassBased { shard: 0 }); // default
    }
