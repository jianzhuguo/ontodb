...
use openraft::{
    Entry, EntryPayload, LogId, LogState, Snapshot, SnapshotMeta, StorageError, StoredMembership,
    Vote,
};
...
impl PersistentRaftStore {
    ...
    fn get_all_sm_data(&self) -> Result<Vec<u8>, std::io::Error> {
        ...
        serde_json::to_vec(&sm_data).map_err(|e| std::io::Error::other(e.to_string()))
    }
}
...
impl RaftStorage<OntoRaftConfig> for PersistentRaftStore {
    ...
    async fn apply_to_state_machine(
        ...
        let last_applied_data =
            serde_json::to_vec(&final_last_applied).map_err(|e| StorageError::IO {
                source: openraft::StorageIOError::new(
        ...
        self.engine
            .put(KEY_LAST_APPLIED.to_vec(), last_applied_data)
            .map_err(|e| StorageError::IO {
                source: openraft::StorageIOError::new(
                    openraft::ErrorSubject::StateMachine,
                    openraft::ErrorVerb::Write,
        ...
        if final_membership != self.last_membership {
            let membership_data =
                serde_json::to_vec(&final_membership).map_err(|e| StorageError::IO {
                    source: openraft::StorageIOError::new(
                        openraft::ErrorSubject::StateMachine,
                        openraft::ErrorVerb::Write,
                        &std::io::Error::other(e.to_string()),
            ...
            self.engine
                .put(KEY_MEMBERSHIP.to_vec(), membership_data)
                .map_err(|e| StorageError::IO {
                    source: openraft::StorageIOError::new(
                        openraft::ErrorSubject::StateMachine,
                        openraft::ErrorVerb::Write,
    ...
    async fn install_snapshot(
        ...
        self.engine
            .put(KEY_SNAPSHOT_META.to_vec(), meta_json)
            .map_err(|e| StorageError::IO {
                source: openraft::StorageIOError::new(
                    openraft::ErrorSubject::Snapshot(None),
                    openraft::ErrorVerb::Write,
    ...
    async fn get_current_snapshot(
        ...
        match self.engine.get(KEY_SNAPSHOT_META) {
            Ok(Some(meta_data)) => {
                match serde_json::from_slice::<SnapshotMeta<NodeId, openraft::BasicNode>>(
                    &meta_data,
                ) {
                    Ok(meta) => {
