...
| 25 | **命名空间隔离** | 多租户全栈隔离(本体/数据/查询/索引/推理)，键前缀路由零开销 |
...
OntoDB 的命名空间不是外挂标签，而是**嵌入存储引擎内核**的全栈隔离机制：

```
传统方案：应用层加 tenant_id 字段 → 查询时 WHERE tenant_id = ? → 容易遗漏
OntoDB：  EntityId = {namespace}::{class}::{pk} → 键前缀路由 → 物理隔离，零开销
```

| 隔离层级 | 说明 |
|----------|------|
| **本体层** | 每个命名空间维护独立的类定义、属性约束、继承关系 |
| **数据层** | 键前缀路由 `{namespace}::` 物理隔离，无交叉 |
| **查询层** | 查询引擎自动应用命名空间上下文，无需 WHERE 过滤 |
| **索引层** | 向量索引、B-Tree 索引、空间索引按命名空间隔离 |
| **推理层** | OWL 推理在命名空间范围内执行，不跨租户推导 |

**优势：** 一个 OntoDB 实例服务 N 个租户/项目，运维成本低，数据安全有保障。
